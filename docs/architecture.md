...
| D-16 | **Architect-Response-Contract + diagnostische JSON-Fehler** (Repair CI/apfel): JSON-Parse-Fehler werden klassifiziert (empty/invalid/truncated/schema violation) und präzise gemeldet („… appears truncated before valid JSON completion“). Ein begrenzter Retry (max. 1 zusätzlicher Request; Note + attempt 2 im Trace) nur bei reparablen Fehlern; bei Truncation kein Retry (Output-Limit; `LLM_MAX_TOKENS` explizit setzen). Architect-Prompt fordert kurze Werte + exaktes JSON-Ende. Kein Reparieren/Raten abgeschnittener JSONs | Repair CI/apfel; Output-Limit war Root Cause der Architect-Truncation |
